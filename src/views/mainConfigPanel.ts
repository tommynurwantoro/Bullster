import { ButtonInteraction, ChatInputCommandInteraction, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle } from 'discord.js';
import { ConfigManager } from '../utils/config';

export async function createMainConfigPanel(interaction: ButtonInteraction | ChatInputCommandInteraction) {
    ...
    const configEmbed = new EmbedBuilder()
        ...
        .setDescription('Click the buttons below to configure different aspects of the bot:')
        .addFields(
            {
                name: '🎯 Welcome System',
                ...
                Message: ${config.welcome.message}` : '❌ Not configured',
                inline: false,
            },
            {
    ...
    const row2 = new ActionRowBuilder()
        .addComponents(
            new ButtonBuilder()
                ...
                .setStyle(config?.points?.marketplaceChannel ? ButtonStyle.Success : config?.points?.logsChannel ? ButtonStyle.Primary : ButtonStyle.Secondary)
                .setEmoji('💰')
...
export async function showMainConfigPanel(interaction: ButtonInteraction, additionalMessage?: string) {
